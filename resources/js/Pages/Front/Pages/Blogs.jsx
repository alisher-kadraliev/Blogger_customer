        ...
        <>
            ...
            <Layout>
                ...
                <div className="container mb-20 px-52">
                    <div className="flex flex-row gap-10 flex-wrap justify-center items-center mx-auto">
                        {cats.map((item, index) => (
                            <Link
                                key={index}
                                className="bg-gray-100 px-5 py-2 rounded-xl hover:text-primary-front font-bold transition-custom hover:border border border-white hover:border-gray-200"
                            >
                                {item.name}
                            </Link>
                        ))}
                ...
                <div className="container">
                    <div className="grid grid-cols-2 mb-20 max-lg:grid-cols-2 px-52 max-lg:px-0 max-md:grid-cols-1 gap-20">
                        {posts.data.map((item, index) => (
                            <Link
                                key={index}
                                href={route("front.blog", { slug: item.slug })}
                                className="flex flex-col mx-auto gap-4 group"
                            >
                                <img
                                    src={item.image_url}
                                    className="border border-gray-500 w-full h-60 object-cover"
                                    alt={item.image_alt}
                                    width={418}
                                    height={240}
                                />
                                ...
                                <p>
                                    Thinking of investing in an online course
                                    and leveling up your web design business?
                                    Before you autofill your credit card details
                                    and click the “Buy Now” button, ask these
                                    three questions to make sure it’s a
